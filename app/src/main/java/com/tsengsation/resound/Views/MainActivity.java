...
public class MainActivity extends FragmentActivity implements ViewFactory, OnPageChangeListener {
    ...
    private void setUpSwipes() {
        mArticlePagerAdapter = new ArticlePagerAdapter(getApplicationContext(),
    ...
    private void setImage(PicassoImageSwitcher imageSwitcher, Article article) {
        ...
        Picasso.with(getApplicationContext()).load(article.imageUrl).into(imageSwitcher);
    }
    ...
    private void initViewReferences() {
        ...
        mArticleImageSwitcherCurr =
                new PicassoImageSwitcher(getApplicationContext(), imageSwitcher1);
        mArticleImageSwitcherPrev =
                new PicassoImageSwitcher(getApplicationContext(), imageSwitcher2);
        mArticleImageSwitcherNext =
                new PicassoImageSwitcher(getApplicationContext(), imageSwitcher3);
    ...
    private void updateFadeY(int yScroll) {
        float alpha;
        float maxOffset = (float) ViewCalculator.dpToPX(this, MAX_ALPHA_OFFSET_DP);
            ...
            if (velocityY < 0) {
                new CountDownTimer(100, 5) {
                    ...
                    public void onTick(long millisUntilFinished) {
                        float maxOffset = (float) ViewCalculator.dpToPX(getApplicationContext(),
                                MAX_ALPHA_OFFSET_DP);
